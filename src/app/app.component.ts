...
export class AppComponent implements OnInit {
  ...
  drillThrough(args: any) {
    debugger
    //Restrict the existing drill through dialog here.
    args.cancel = true;
    this.eventArgs = args;
    ...
    if (!document.body.querySelector('#' + this.pivotObj?.element.id + 'inline')) {
      // Create a numeric input box for editing the specific cells.
  ...
  onChange(args: any) {
    ...
    } // If the updated value is lesser than the previous value, split those values and subtracted it to the existing raw data here.
    else {
  ...
  onBlur(args: any) {
    ...
    // You can get the updated pivot values here.
    if (this.pivotObj) {
